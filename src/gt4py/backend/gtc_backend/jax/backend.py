...
from gtc.jax.jaxir_codegen import JaxirCodegen
from gtc.jax.oir_to_jaxir import OirToJaxir
...
class GTCModuleGenerator(BaseModuleGenerator):
    ...
    @property
    def backend(self) -> "GTCJaxBackend":
        return cast(GTCJaxBackend, self.builder.backend)
...
@register
class GTCJaxBackend(BaseBackend, CLIBackendMixin):
    """JAX backend using gtc."""
    ...
    def generate_computation(self) -> Dict[str, Union[str, Dict]]:
        ...
        source = JaxirCodegen.apply(self.jaxir)
    ...
    def _make_jaxir(self) -> jaxir.Computation:
        ...
        return OirToJaxir().visit(oir)

    @property
    def jaxir(self) -> jaxir.Computation:
        key = "gtcjax:jaxir"
        if key not in self.builder.backend_data:
            self.builder.with_backend_data({key: self._make_jaxir()})
