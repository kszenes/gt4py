...
from gtc.jax import jaxir


__all__ = ["JaxirCodegen"]
...
def slice_to_extent(acc: jaxir.FieldSlice) -> Extent:
    return Extent(
        (
...
class ExtentCalculator(NodeVisitor):
    ...
    def visit_Computation(self, node: jaxir.Computation):
        ctx = self.Context()
    ...
    def visit_VerticalPass(self, node: jaxir.VerticalPass, *, ctx: Context):
        for block in reversed(node.body):
            self.visit(block, ctx=ctx)

    def visit_HorizontalBlock(self, node: jaxir.HorizontalBlock, *, ctx: Context):
        writes = (
            node.iter_tree()
            .if_isinstance(jaxir.VectorAssign)
            .getattr("left")
            .if_isinstance(jaxir.FieldSlice)
            .getattr("name")
            .to_set()
        )
...
class JaxirCodegen(TemplatedGenerator):
    ...
    def visit_NumericalOffset(
        self, node: jaxir.NumericalOffset, **kwargs: Any
    ...
    def visit_AxisOffset(
        self, node: jaxir.AxisOffset, **kwargs: Any
    ) -> Union[str, Collection[str]]:
    ...
    def visit_FieldDecl(self, node: jaxir.FieldDecl, **kwargs) -> Union[str, Collection[str]]:
    ...
    def visit_VariableKOffset(
        self, node: jaxir.VariableKOffset, **kwargs: Any
    ) -> Union[str, Collection[str]]:
    ...
    def visit_EmptyTemp(
        self, node: jaxir.EmptyTemp, *, temp_name: str, **kwargs
    ) -> Union[str, Collection[str]]:
        shape = "_domain_"
        origin = [0, 0, 0]
    ...
    def visit_MaskBlock(self, node: jaxir.MaskBlock, **kwargs) -> Union[str, Collection[str]]:
        if isinstance(node.mask, jaxir.FieldSlice):
            mask_def = ""
        elif isinstance(node.mask, jaxir.BroadCast):
    ...
    def visit_VectorAssign(
        self, node: jaxir.VectorAssign, **kwargs: Any
    ) -> Union[str, Collection[str]]:
        mask_acc = ""
        if node.mask:
            mask_acc = f"[{self.visit(node.mask, **kwargs)}]"
        if isinstance(node.right, jaxir.EmptyTemp):
    ...
    def visit_VerticalPass(self, node: jaxir.VerticalPass, **kwargs):
        return self.generic_visit(
    ...
    def visit_Computation(
        self, node: jaxir.Computation, **kwargs: Any
    ) -> Union[str, Collection[str]]:
